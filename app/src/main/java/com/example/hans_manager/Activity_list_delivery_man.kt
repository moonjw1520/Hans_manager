...
class Activity_list_delivery_man : AppCompatActivity() {
    ...
    private lateinit var m_Adapter : CAdapter_list_delivery
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_Ok.setOnClickListener {
            ...
            (0 until  m_Adapter.count)
                    ...
                    .forEach { send_sms_list.add(D_DeliveryPerson(list_delivery_man[it].id,list_delivery_man[it].name, list_delivery_man[it].mdn,  "0", "미정") )
                     list_delivery_man[it].List_Event.add(D_Event(CListCollector.list_event[m_nEvent_Index].id,
                    ...
                    for(inx in 0 until  send_sms_list.size)
                    {
                        ...
                        //DB에 매칭을 저장한다.
                        //InsertMatching(it,send_sms_list.get(inx).id)
                    }
            DeleteMatching()
        ...
        m_Adapter =  CAdapter_list_delivery(this@Activity_list_delivery_man ,list_delivery_man, m_nEvent_Index, m_event_id)
