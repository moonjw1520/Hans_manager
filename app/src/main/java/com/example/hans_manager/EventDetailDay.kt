...
import kotlinx.android.synthetic.main.activity_event_detail_day.*

class EventDetailDay : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mEvenDate = intent.getStringExtra("eventDate")
            ...
            if(mEvenDate == CListCollector.list_event[i].event_date)
            {
                ...
                if(CListCollector.list_event[i].send_sms == "1")
                {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        txtView_sms_state.setAutoSizeTextTypeWithDefaults(AUTO_SIZE_TEXT_TYPE_UNIFORM)
                    }
